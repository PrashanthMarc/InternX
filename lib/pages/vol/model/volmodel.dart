class VolFeedModel {
  ...
  VolFeedModel({this.feeds, this.appData, this.userDetails});

  VolFeedModel.fromJson(Map<String, dynamic> json) {
    if (json['feeds'] != null) {
      feeds = new List<VolFeeds>();
      json['feeds'].forEach((v) {
        feeds.add(new VolFeeds.fromJson(v));
      });
    }
...
class VolFeeds {
  ...
  VolFeeds(
      {this.id, this.user, this.info, this.body, this.userPic, this.iamge});

  VolFeeds.fromJson(Map<String, dynamic> json) {
    id = json['id'];
    user = json['user'];
    info = json['info'];
    body = json['body'];
